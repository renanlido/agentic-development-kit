# adk-v2-fase3 Context

Inherits: .claude/memory/project-context.md

## Feature-specific Context

Vamos implementar a fase 3 agora, leia a documentação
...
**Data:** 2026-01-21
**Status:** Validado Contra Codigo + Code Review + Autonomous Discovery + Git Checkpoints + Resilience & Observability + Tasks por Fase
**Versao:** 4.3
...
echo "2. **State Sync**: Keep progress.md ↔ tasks/ synchronized"
...
### 2. State Management
- Keep progress.md ↔ tasks/ synchronized (arquivos por fase)
...
#### 9.11.11 Arquitetura de Tasks em Pasta (tasks/)

**Problema com tasks.md único:**

```
┌─────────────────────────────────────────────────────────────────────────┐
│              PROBLEMA: tasks.md MONOLITICO                               │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                          │
│  tasks.md (arquivo unico):                                              │
│  ├─ Todas as fases misturadas em um arquivo                            │
│  ├─ Dificil rastrear progresso por fase                                │
│  ├─ Context window carrega tudo mesmo quando irrelevante               │
│  ├─ Conflitos de merge quando multiplos agentes editam                 │
│  └─ Memory fading: informacao se perde no meio do arquivo              │
│                                                                          │
│  tasks/ (pasta com arquivos por fase):                                  │
│  ├─ Separacao clara por fase de desenvolvimento                        │
│  ├─ Progresso rastreavel por arquivo individual                        │
│  ├─ Context loading apenas da fase atual                               │
│  ├─ Sem conflitos: cada agente trabalha em arquivo diferente           │
│  └─ Agente especializado cria tasks estruturadas                       │
│                                                                          │
└─────────────────────────────────────────────────────────────────────────┘
```
...
├── progress.md              # Status geral da feature
├── research.md              # Fase de pesquisa
├── prd.md                   # Product requirements
├── constraints.md           # Restricoes de escopo
├── state.json               # Estado unificado
├── history.json             # Historico de transicoes
├── metrics.json             # Metricas coletadas
├── claude-progress.txt      # Log para long-running agents
└── tasks/                   # NOVO: Pasta de tasks por fase
    ├── phase-0-setup.md           # Fase 0: Setup inicial (JA FEITA)
    ├── phase-1-research.md        # Fase 1: Research
    ├── phase-2-planning.md        # Fase 2: Planning
    ├── phase-3-implementation.md  # Fase 3: Implementation
    ├── phase-4-testing.md         # Fase 4: Testing
    ├── phase-5-qa.md              # Fase 5: QA
    └── phase-6-deploy.md          # Fase 6: Deploy
```

**Formato de Arquivo de Task (phase-N-name.md):**
...
### Task 1: Descricao da Task
- **Status:** [x] completed
- **Started:** 2026-01-20T10:00:00Z
- **Completed:** 2026-01-20T10:30:00Z
- **Commit:** abc123
- **Files:**
  - `src/utils/example.ts` (created)
  - `tests/example.test.ts` (created)

### Task 2: Outra Task
- **Status:** [~] in_progress
- **Started:** 2026-01-20T10:35:00Z
- **Notes:** Aguardando review do PR #42

### Task 3: Task Pendente
- **Status:** [ ] pending
- **Depends on:** Task 2
- **Estimated:** 2h
...
Anotacoes especificas desta fase...
```

**Agente Especializado para Criacao de Tasks:**

```markdown
---
name: task-breakdown-agent
description: Cria arquivos de tasks estruturados por fase
context: fork
...
## Objetivo
Analisar PRD e research para criar arquivos de tasks estruturados em `.claude/plans/features/<name>/tasks/`.

## Workflow

1. **Ler contexto:**
   - Ler PRD em prd.md
   - Ler research em research.md
   - Identificar fases necessarias

2. **Para cada fase:**
   - Criar arquivo `tasks/phase-N-name.md`
   - Extrair tasks do PRD/research
   - Definir dependencias entre tasks
   - Estimar esforco
   - Definir acceptance criteria

3. **Validar estrutura:**
   - Verificar que todas as tasks tem status
   - Verificar que dependencias sao validas
   - Verificar que estimativas sao realistas

## Output
Pasta `tasks/` com arquivos por fase, prontos para execucao.
```

**CLI para Gerenciar Tasks por Fase:**

```bash
# Listar tasks de uma fase especifica
adk feature tasks <name> --phase 3

# Ver progresso por fase
adk feature status <name> --by-phase

# Marcar task como concluida
adk feature task-done <name> --phase 3 --task 2

# Criar tasks a partir de PRD (usando agente)
adk feature breakdown <name>

# Ver proxima task a fazer
adk feature next-task <name>
```

**Integracao com StateManager:**

```typescript
// src/utils/state-manager.ts - Novos metodos
export class StateManager {
  async loadPhaseState(feature: string, phase: number): Promise<PhaseState> {
    const phasePath = this.getTasksPath(feature, phase)
    const content = await fs.readFile(phasePath, 'utf-8')
    return this.parsePhaseMarkdown(content)
  }
  ...
  async updateTaskStatus(
    ...
    const state = await this.loadPhaseState(feature, phase)
    state.tasks[taskNumber - 1].status = status

    if (metadata?.commit) {
      state.tasks[taskNumber - 1].commit = metadata.commit
    }
    if (metadata?.files) {
      state.tasks[taskNumber - 1].files = metadata.files
    }

    await this.savePhaseState(feature, phase, state)
    await this.updateOverallProgress(feature)
  }

  async getNextTask(feature: string): Promise<{phase: number, task: number} | null> {
    const phases = await this.listPhases(feature)

    for (const phase of phases) {
      const state = await this.loadPhaseState(feature, phase.number)
      const nextTask = state.tasks.findIndex(t => t.status === 'pending')

      if (nextTask !== -1) {
        return { phase: phase.number, task: nextTask + 1 }
      }
    }
  ...
  private async updateOverallProgress(feature: string): Promise<void> {
    const phases = await this.listPhases(feature)
    let totalTasks = 0
    ...
    await this.updateUnifiedState(feature, { progress })
  }
}
...
interface PhaseState {
  ...
  tasks: TaskState[]
  acceptanceCriteria: CriteriaState[]
}
...
**Hook para Atualizar Progresso por Fase:**

```bash
#!/bin/bash
# .claude/hooks/update-phase-progress.sh
# Atualiza progresso da fase atual apos task completion
...
FEATURE=$(grep "^feature:" "$FOCUS_FILE" | cut -d':' -f2 | xargs)
CURRENT_PHASE=$(grep "^phase:" "$FOCUS_FILE" | cut -d':' -f2 | xargs)

if [ -z "$FEATURE" ] || [ -z "$CURRENT_PHASE" ]; then
  exit 0
fi

TASKS_DIR="$PROJECT_DIR/.claude/plans/features/$FEATURE/tasks"
PHASE_FILE="$TASKS_DIR/phase-${CURRENT_PHASE}-*.md"

# Contar tasks completadas vs total
if ls $PHASE_FILE 1> /dev/null 2>&1; then
  TOTAL=$(grep -c "^### Task" $PHASE_FILE || echo "0")
  COMPLETED=$(grep -c "\[x\] completed" $PHASE_FILE || echo "0")
  PROGRESS=$((COMPLETED * 100 / TOTAL))

  # Atualizar progress no arquivo
  sed -i '' "s/Progress:.*/Progress: $COMPLETED\/$TOTAL tasks ($PROGRESS%)/" $PHASE_FILE

  # Se todas completadas, marcar fase como completed
  if [ "$COMPLETED" -eq "$TOTAL" ]; then
    sed -i '' "s/Status:.*/Status: completed/" $PHASE_FILE
    sed -i '' "s/Completed:.*/Completed: $(date -Iseconds)/" $PHASE_FILE
  fi
fi
...
**Fluxo Completo com Tasks por Fase:**

```
┌─────────────────────────────────────────────────────────────────────────┐
│              FLUXO: TASKS POR FASE COM AGENTE ESPECIALIZADO              │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                          │
│  1. CRIACAO (adk feature breakdown <name>)                              │
│     ├─ Agente task-breakdown-agent le PRD + research                   │
│     ├─ Cria arquivos tasks/phase-N-*.md para cada fase                 │
│     └─ Define tasks, dependencias, estimativas                         │
│                                                                          │
│  2. EXECUCAO (adk feature next-task <name>)                             │
│     ├─ StateManager identifica proxima task pendente                   │
│     ├─ Carrega apenas o arquivo da fase atual (context lean)           │
│     └─ Agente implementador executa a task                             │
│                                                                          │
│  3. COMPLETION (hook update-phase-progress.sh)                          │
│     ├─ Marca task como [x] completed no arquivo da fase                │
│     ├─ Atualiza Progress: X/Y tasks (Z%)                               │
│     ├─ Cria commit atomico                                             │
│     └─ Se fase 100%: marca fase como completed                         │
│                                                                          │
│  4. TRANSICAO (automatica)                                              │
│     ├─ Quando fase N completa, avanca para fase N+1                    │
│     ├─ Carrega proximo arquivo tasks/phase-(N+1)-*.md                  │
│     └─ Repete ciclo ate todas as fases completadas                     │
│                                                                          │
└─────────────────────────────────────────────────────────────────────────┘
```

**Tarefas de Implementacao - Tasks em Pasta:**

| # | Task | Arquivo | Prioridade | Esforco |
|---|------|---------|------------|---------|
| 1 | Criar agente task-breakdown-agent | `.claude/agents/task-breakdown.md` | P0 | 2h |
| 2 | Implementar loadPhaseState no StateManager | `src/utils/state-manager.ts` | P0 | 3h |
| 3 | Implementar updateTaskStatus | `src/utils/state-manager.ts` | P0 | 2h |
| 4 | Implementar getNextTask | `src/utils/state-manager.ts` | P1 | 1h |
| 5 | CLI `adk feature breakdown` | `src/commands/feature.ts` | P1 | 3h |
| 6 | CLI `adk feature tasks --phase N` | `src/commands/feature.ts` | P1 | 2h |
| 7 | CLI `adk feature next-task` | `src/commands/feature.ts` | P1 | 1h |
| 8 | Hook update-phase-progress.sh | `.claude/hooks/update-phase-progress.sh` | P1 | 1h |
| 9 | Template de arquivo de fase | `templates/task-phase.md` | P2 | 30min |
| 10 | Testes para StateManager phase methods | `tests/state-manager-phases.test.ts` | P1 | 3h |
...
| **v4.3** | **Arquitetura de tasks em pasta com agente especializado** |
...
**Novo em v4.3:**
- Secao 9.11.11: Arquitetura de tasks em pasta (tasks/)
- Substituicao de tasks.md monolitico por pasta com arquivos por fase
- Agente especializado task-breakdown-agent para criar tasks estruturadas
- Formato padronizado para arquivos phase-N-name.md
- Novos metodos no StateManager: loadPhaseState, updateTaskStatus, getNextTask
- Novos comandos CLI: breakdown, tasks --phase, next-task, task-done
- Hook update-phase-progress.sh para atualizacao automatica
- Fluxo completo de criacao → execucao → completion → transicao
- 10 novas tarefas de implementacao (tasks em pasta)
- Nota: Phase 0 (setup) ja feita - estrutura comeca a partir da Phase 1

---

**Documento atualizado em:** 2026-01-21
**Versao:** 4.3 (com Tasks por Fase)
